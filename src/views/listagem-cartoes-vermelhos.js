...
function ListagemVermelhos() {
  ...
  const cadastrar = () => {
    navigate('/cadastro-cartoes-vermelhos');
  };

  const editar = (id) => {
    navigate(`/cadastro-cartpes-vermelhos/${id}`);
  };

  function gerarRanking(nomeCompeticao) {
    const vermelhosDaCompeticao = dadosOriginais.filter(
      (dado) => dado.nomeCompeticao === nomeCompeticao
    );

    const mapa = {};

    vermelhosDaCompeticao.forEach((item) => {
      ...
      mapa[idJogador].totalVermelhos += 1;
    });

    const rankingOrdenado = Object.values(mapa).sort(
      (a, b) => b.totalVermelhos - a.totalVermelhos
    );
  ...
  async function excluir(id) {
    try {
      ...
      mensagemSucesso('Cartão vermelho excluído com sucesso!');
      ...
      mensagemErro('Erro ao excluir cartão vermelho');
    }
  }
  ...
  return (
    <div className="container">
      <Card title="Listagem de Vermelhos">
        <div className="row">
          <div className="col-lg-12">
            ...
            {competicaoSelecionada && ranking.length > 0 && (
              <div className="mt-5">
                <h4>
                  Jogadores com mais vermelhos — {competicaoSelecionada}
                </h4>
...
export default ListagemVermelhos;